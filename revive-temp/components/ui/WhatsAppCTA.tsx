 ...
 * Number and message are dynamically built — never hard-coded.
 * Falls back to null render if no number configured.
 */
export function WhatsAppCTA({
  ...
  const resolvedMessage: string =
    ...
    (context === 'program' && programName
      ? WHATSAPP_MESSAGES.program(programName)
      : (WHATSAPP_MESSAGES[context as Exclude<typeof context, 'program'>] as string))

  const url = buildWhatsAppUrl(whatsappNumber, resolvedMessage)
  ...
  const variantStyles = {
    ...
    ghost: 'text-[#25D366] hover:text-white',
    'icon-only': 'text-[#25D366] hover:text-white p-2',
  }
