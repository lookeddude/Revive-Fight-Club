...
export default async function TrialDetailPage({
  ...
  const waUrl = `https://wa.me/${trial.phone.replace(/\D/g, '')}?text=${waMessage}`

  return (
    <div className="max-w-3xl space-y-6">
      {/* Back */}
      <Link href="/admin/trials" className="inline-flex items-center gap-2 font-[family-name:var(--font-inter)] text-xs text-[#6b7280] hover:text-[#9ca3af] transition-colors uppercase tracking-wider">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="font-[family-name:var(--font-inter)] text-sm text-[#6b7280] mt-1">Submitted {formatDate(trial.created_at)}</p>
        </div>
      ...
      <div className="flex flex-wrap gap-3">
        <a
          ...
          className="inline-flex items-center gap-2 border border-white/[0.08] px-4 py-2 font-[family-name:var(--font-inter)] text-xs font-bold uppercase tracking-wider text-[#9ca3af] hover:border-white/20 hover:text-[#e2e3e1] transition-colors"
        >
