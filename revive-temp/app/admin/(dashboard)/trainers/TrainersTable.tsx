...
export function TrainersTable({ trainers }: { trainers: Trainer[] }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="bg-[#111312] border border-white/[0.07]">
        {filtered.length === 0 ? (
          <div className="text-center py-12">
            <p className="font-[family-name:var(--font-inter)] text-sm text-[#4b5563]">No trainers match &quot;{search}&quot;</p>
          </div>
        ) : (
              ...
              <tbody>
                {filtered.map(t => (
                  <tr key={t.id} className="border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-4 py-3 font-[family-name:var(--font-inter)] text-xs text-[#4b5563]">{t.years_experience ? `${t.years_experience} yrs` : '—'}</td>
                    <td className="px-4 py-3"><StatusBadge status={t.is_active ? 'active' : 'inactive'} /></td>
                    <td className="px-4 py-3">{t.is_featured && <span className="font-[family-name:var(--font-inter)] text-[10px] font-bold uppercase tracking-wider text-[#ff571a] border border-[#ff571a]/30 px-2 py-0.5">Featured</span>}</td>
      ...
      {search && <p className="font-[family-name:var(--font-inter)] text-xs text-[#4b5563]">{filtered.length} of {trainers.length} trainers</p>}
    </div>
  )
}
