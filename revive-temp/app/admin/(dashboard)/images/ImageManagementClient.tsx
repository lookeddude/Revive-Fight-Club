      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {statCards.map(card => (
          <div key={card.label} className="px-4 py-3" style={{ background: '#0f1110', border: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            <div className="font-[family-name:var(--font-body)] text-sm text-[#4b5563] uppercase tracking-wider mt-0.5">
              {card.label}
            </div>
          </div>
        ))}
      ...
      <div className="flex gap-2 flex-wrap">
        {SECTIONS.map(s => (
          <button
            ...
            className={`px-4 py-1.5 font-[family-name:var(--font-body)] text-xs font-bold uppercase tracking-wider transition-colors border ${
              section === s
                ? s === 'Hero Slideshow'
                  ? 'bg-[#ff571a] text-black border-[#ff571a]'
                  : 'bg-[#ff571a] text-black border-[#ff571a]'
                : 'border-white/[0.08] text-[#9ca3af] hover:text-[#f0ede8] hover:border-white/20'
            }`}
          >
        ...
        <>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-[#0f1110] border border-white/[0.07] text-sm text-[#f0ede8] focus:outline-none focus:border-[#ff571a]/40 font-[family-name:var(--font-body)] placeholder:text-[#3a3530]"
            />
          </div>
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-16 border border-dashed border-white/[0.06]">
              <p className="font-[family-name:var(--font-body)] text-sm text-[#4b5563]">No slots match your search.</p>
            </div>
          ) : (
