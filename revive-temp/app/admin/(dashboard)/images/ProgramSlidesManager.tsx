...
export function ProgramSlidesManager({ initialPrograms }: Props) {
  ...
  const ic = 'bg-[#0a0b0a] border border-white/[0.08] px-3 py-2 text-sm text-[#e2e3e1] focus:outline-none focus:border-[#ff571a]/50 w-full font-[family-name:var(--font-body)] placeholder:text-[#3a3530]'
  const btn = 'px-3 py-1.5 font-[family-name:var(--font-body)] text-xs font-bold uppercase tracking-wider transition-colors'
          ...
          <div className="w-full max-w-lg" style={{ background: '#111312', border: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            <div className="px-5 py-5 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="font-[family-name:var(--font-body)] text-xs text-[#4b5563] uppercase tracking-wider block mb-1">Name *</label>
                ...
                <div>
                  <label className="font-[family-name:var(--font-body)] text-xs text-[#4b5563] uppercase tracking-wider block mb-1">Slug *</label>
                  <input className={ic} value={form.slug} onChange={e => setForm(f => ({ ...f, slug: toSlug(e.target.value) }))} placeholder="e.g. mma" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="font-[family-name:var(--font-body)] text-xs text-[#4b5563] uppercase tracking-wider block mb-1">Category</label>
                  <input className={ic} value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} placeholder="e.g. Combat Sports" />
                </div>
                <div>
                  <label className="font-[family-name:var(--font-body)] text-xs text-[#4b5563] uppercase tracking-wider block mb-1">Level</label>
              ...
              <div>
                <label className="font-[family-name:var(--font-body)] text-xs text-[#4b5563] uppercase tracking-wider block mb-1">Duration (minutes)</label>
              ...
              <div>
                <label className="font-[family-name:var(--font-body)] text-xs text-[#4b5563] uppercase tracking-wider block mb-1">Short Description</label>
              ...
              <div className="flex gap-3 pt-1">
                <button onClick={handleSaveProgram} disabled={saving} className="flex-1 bg-[#ff571a] text-black font-[family-name:var(--font-body)] text-xs font-black uppercase tracking-wider py-2.5 hover:bg-white transition-colors disabled:opacity-50">
                  {saving ? 'Saving...' : editingProgram ? 'Save Changes' : 'Create Program'}
                </button>
                <button onClick={() => setShowAddProgram(false)} className="px-5 border border-white/10 text-[#9ca3af] font-[family-name:var(--font-body)] text-xs font-bold uppercase tracking-wider hover:text-[#f0ede8] hover:border-white/20 transition-colors">
                  Cancel
                </button>
              </div>
            </div>
      ...
      <div className="flex gap-3 items-start flex-wrap md:flex-nowrap">
        <div className="w-full md:w-60 shrink-0 space-y-1">
          <div className="flex items-center justify-between mb-2">
            <p className="font-[family-name:var(--font-body)] text-sm text-[#4b5563] uppercase tracking-wider">Programs</p>
            <button onClick={openAdd} className="flex items-center gap-1 px-2 py-1 bg-[#ff571a] text-black font-[family-name:var(--font-body)] text-xs font-black uppercase tracking-wider hover:bg-white transition-colors">
              <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 4v16m8-8H4" /></svg>
              Add
            </button>
          </div>
          {programs.length === 0 && (
            <p className="font-[family-name:var(--font-body)] text-xs text-[#4b5563] py-4 text-center border border-dashed border-white/[0.06]">No programs. Click Add to create one.</p>
          )}
          {programs.map(p => (
            <div key={p.id} className={`group flex items-center gap-1 ${selectedId === p.id ? 'bg-[#ff571a]' : 'bg-[#0a0b0a] border border-white/[0.07] hover:border-white/20'}`}>
              <button onClick={() => setSelectedId(p.id)} className={`flex-1 text-left px-3 py-2.5 text-sm font-[family-name:var(--font-body)] transition-colors flex items-center justify-between gap-2 ${selectedId === p.id ? 'text-black font-bold' : 'text-[#9a9088]'}`}>
                <span className="truncate">{p.name}</span>
                <span className={`shrink-0 text-xs font-bold px-1.5 py-0.5 rounded ${selectedId === p.id ? 'bg-black/20 text-black' : 'bg-white/5 text-[#9ca3af]'}`}>{p.slides.filter(s => s.is_active).length}</span>
              </button>
        ...
        <div className="flex-1 min-w-0 space-y-4">
          {!selected ? (
            <div className="text-center py-16 border border-dashed border-white/[0.06]">
              <p className="font-[family-name:var(--font-body)] text-sm text-[#4b5563]">Create a program first to manage its slides.</p>
            </div>
          ) : (
            <>
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div>
                  ...
                  <p className="font-[family-name:var(--font-body)] text-xs text-[#4b5563] mt-0.5">{selected.slides.length} slide{selected.slides.length !== 1 ? 's' : ''} - max 10</p>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => openEdit(selected)} className="flex items-center gap-1.5 px-3 py-1.5 border border-white/[0.08] text-[#9a9088] hover:text-[#f0ede8] hover:border-white/20 font-[family-name:var(--font-body)] text-xs font-bold uppercase tracking-wider transition-colors">
                    <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                    Edit Program
                  </button>
                  <span className="font-[family-name:var(--font-body)] text-xs text-[#4b5563] border border-white/[0.07] px-2 py-1 uppercase tracking-wider">/{selected.slug}</span>
                </div>
              </div>
              ...
              <div style={{ background: '#0a0b0a', border: '1px solid rgba(255,255,255,0.06)' }} className="p-4 space-y-3">
                <div className="flex gap-2">
                  {(['upload', 'url'] as const).map(tab => (
                    <button key={tab} onClick={() => setAddTab(tab)} className={`${btn} ${addTab === tab ? 'bg-[#ff571a] text-black' : 'border border-white/10 text-[#9ca3af] hover:text-[#f0ede8]'}`}>
                ...
                {addTab === 'upload' ? (
                  <div>
                    ...
                    <button onClick={() => fileRef.current?.click()} disabled={uploading || selected.slides.length >= 10} className="w-full border-2 border-dashed border-white/10 hover:border-[#ff571a]/40 transition-colors py-8 flex flex-col items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed">
                      {uploading ? (<><div className="w-5 h-5 border-2 border-[#ff571a] border-t-transparent rounded-full animate-spin" /><span className="font-[family-name:var(--font-body)] text-xs text-[#9ca3af]">Uploading...</span></>) : (
                        <><svg className="w-8 h-8 text-[#3a3530]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg><span className="font-[family-name:var(--font-body)] text-sm text-[#9ca3af]">{selected.slides.length >= 10 ? 'Max 10 slides reached' : 'Click to upload photo'}</span><span className="font-[family-name:var(--font-body)] text-xs text-[#3a3530]">JPG, PNG, WebP - max 10MB</span></>
                      )}
                    </button>
                  </div>
                ) : (
                  <div className="flex gap-2">
                    ...
                    <button onClick={handleUrlAdd} disabled={!urlInput.trim() || selected.slides.length >= 10} className="bg-[#ff571a] text-black font-[family-name:var(--font-body)] text-xs font-black uppercase tracking-wider px-4 py-2 hover:bg-white transition-colors disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap">Add</button>
                  </div>
                )}
              </div>
              ...
              {selected.slides.length === 0 ? (
                <div className="text-center py-12 border border-dashed border-white/[0.06]">
                  <p className="font-[family-name:var(--font-body)] text-sm text-[#4b5563]">No slides yet - upload images or paste URLs above.</p>
                </div>
              ) : (
                  ...
                  {selected.slides.map((slide, idx) => (
                    <div key={slide.id} className="relative group" style={{ opacity: slide.is_active ? 1 : 0.45 }}>
                      <div className="relative aspect-[4/3] overflow-hidden" style={{ background: '#111' }}>
                        ...
                        <div className="absolute top-1.5 left-1.5 w-5 h-5 rounded-full bg-black/70 flex items-center justify-center"><span className="font-[family-name:var(--font-body)] text-xs font-bold text-white">{idx + 1}</span></div>
                        {!slide.is_active && (<div className="absolute top-1.5 right-1.5 bg-[#3a3530] px-1.5 py-0.5"><span className="font-[family-name:var(--font-body)] text-xs uppercase tracking-wider text-[#9ca3af]">Hidden</span></div>)}
                      ...
                      <p className="font-[family-name:var(--font-body)] text-xs text-[#4b5563] mt-1 truncate px-0.5">Slide {idx + 1} {!slide.is_active ? '- Hidden' : '- Visible'}</p>
                    </div>
                  ))}
                </div>
              )}
              {selected.slides.length > 0 && (
                <p className="font-[family-name:var(--font-body)] text-xs text-[#4b5563]">Slides show in order on the <a href={`/programs/${selected.slug}`} target="_blank" className="text-[#ff571a] hover:underline">{selected.name}</a> page.</p>
              )}
            </>
          )}
        </div>
