    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { if (showAdd) { setShowAdd(false); resetForm() } else setShowAdd(true) }}
          className="bg-[#ff571a] text-black font-[family-name:var(--font-body)] text-xs font-bold uppercase tracking-wider px-4 py-2 hover:bg-white transition-colors shrink-0">
      ...
      {showAdd && (
        <form onSubmit={handleSubmit} className="bg-[#111312] border border-white/[0.08] p-5 space-y-4">
          <h3 className="font-[family-name:var(--font-body)] text-xs font-bold uppercase tracking-wider text-[#9ca3af]">Upload Gallery Image</h3>
          ...
          <div className="flex justify-end">
            <button type="submit" disabled={saving || uploading || !imagePath} className="bg-[#ff571a] text-black font-[family-name:var(--font-body)] text-xs font-bold uppercase tracking-wider px-4 py-2 hover:bg-white transition-colors disabled:opacity-50">{saving ? 'Saving…' : 'Save to Gallery'}</button>
          </div>
        </form>
      )}
            ...
            return (
              <div key={item.id} className="group relative bg-[#111312] border border-white/[0.08] overflow-hidden">
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                  <button onClick={() => setArchiveTarget(item.id)} className="font-[family-name:var(--font-body)] text-xs font-bold uppercase text-red-400 hover:text-red-300">Archive</button>
                </div>
                <div className="p-2">
                  {item.title && <p className="font-[family-name:var(--font-body)] text-xs text-[#9ca3af] truncate">{item.title}</p>}
                  <div className="flex items-center gap-1.5 mt-1">
                    <span className="font-[family-name:var(--font-body)] text-xs text-[#4b5563] capitalize">{item.category}</span>
