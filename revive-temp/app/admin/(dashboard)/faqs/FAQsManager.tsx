    ...
    <div className="space-y-5">
      ...
      {showAdd && (
        <form onSubmit={handleSubmit} className="bg-[#111312] border border-white/[0.08] p-5 space-y-4">
          <h3 className="font-[family-name:var(--font-body)] text-xs font-bold uppercase tracking-wider text-[#9ca3af]">{editItem ? 'Edit FAQ' : 'New FAQ'}</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="flex items-center pt-5"><label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={isPublished} onChange={e => setIsPublished(e.target.checked)} className="w-4 h-4 accent-[#ff571a]" /><span className="font-[family-name:var(--font-body)] text-sm text-[#e2e3e1]">Published</span></label></div>
          </div>
          <div className="flex justify-end">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    {faq.category && <span className="font-[family-name:var(--font-body)] text-xs font-bold uppercase tracking-wider text-[#6b7280] border border-white/[0.08] px-2 py-0.5">{faq.category}</span>}
                    <StatusBadge status={faq.is_published ? 'published' : 'draft'} />
                  </div>
                  <p className="font-[family-name:var(--font-body)] text-sm font-medium text-[#e2e3e1] mb-1">{faq.question}</p>
                  <p className="font-[family-name:var(--font-body)] text-sm text-[#9ca3af] line-clamp-2">{faq.answer}</p>
                </div>
                <div className="flex gap-3 flex-shrink-0">
                  <button onClick={() => handleEdit(faq)} className="font-[family-name:var(--font-body)] text-xs font-bold uppercase tracking-wider text-[#ff571a] hover:text-white transition-colors">Edit</button>
                  <button onClick={() => setDeleteTarget(faq.id)} className="font-[family-name:var(--font-body)] text-xs font-bold uppercase tracking-wider text-red-500/60 hover:text-red-400 transition-colors">Delete</button>
                </div>
              </div>
            </div>
          ))}
