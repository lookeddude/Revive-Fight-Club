    ...
    <div className="max-w-4xl">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="font-[family-name:var(--font-inter)] text-sm text-[#6b7280] mt-1">Manage staff access and send invitations.</p>
        </div>
      ...
      {actionError && (
        <div className="mb-4 p-3 border border-red-500/30 bg-red-500/10 text-red-400 text-sm font-[family-name:var(--font-inter)]">{actionError}</div>
      )}

      {/* Current Staff */}
      <section className="mb-10">
        <h2 className="font-[family-name:var(--font-inter)] text-xs font-bold tracking-[0.15em] uppercase text-[#6b7280] mb-4">Current Staff ({staff.length})</h2>
                  ...
                  <tr key={member.id} className="border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors">
                    <td className="px-4 py-3">
                      <span className="font-[family-name:var(--font-inter)] text-sm text-[#e2e3e1]">
                        {member.full_name ?? '\u2014'}
                        {member.id === currentProfile.id && <span className="ml-2 text-[10px] text-[#6b7280]">(you)</span>}
                      </span>
                    </td>
                    <td className="px-4 py-3"><span className="font-[family-name:var(--font-inter)] text-sm text-[#bab8b7]">{member.email || '\u2014'}</span></td>
                    <td className="px-4 py-3"><RoleBadge role={member.role} /></td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex px-2 py-0.5 text-[10px] font-bold tracking-[0.1em] uppercase border font-[family-name:var(--font-inter)] ${member.is_active ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' : 'bg-red-500/20 text-red-400 border-red-500/30'}`}>
                    ...
                    {canInvite(currentProfile.role) && (
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-3">
                          {canToggle(member) && (
                            <button onClick={() => handleToggleStatus(member.id, member.is_active)} disabled={isPending} className="font-[family-name:var(--font-inter)] text-xs font-bold tracking-[0.08em] uppercase text-[#6b7280] hover:text-[#e2e3e1] transition-colors disabled:opacity-40">
                              {member.is_active ? 'Deactivate' : 'Activate'}
                            </button>
                          )}
                          {canDeleteMember(member) && (
                            <button onClick={() => { setDeleteTarget(member); setDeleteConfirmText('') }} disabled={isPending} className="font-[family-name:var(--font-inter)] text-xs font-bold tracking-[0.08em] uppercase text-red-500 hover:text-red-400 transition-colors disabled:opacity-40">
                              Delete
                            </button>
                          )}
                        </div>
      ...
      {canInvite(currentProfile.role) && (
        <section className="mb-10">
          <h2 className="font-[family-name:var(--font-inter)] text-xs font-bold tracking-[0.15em] uppercase text-[#6b7280] mb-4">Pending Invitations ({pendingInvites.length})</h2>
          <div className="border border-white/[0.08] overflow-hidden">
                ...
                <tbody>
                  {pendingInvites.map((inv) => (
                    <tr key={inv.id} className="border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors">
                      <td className="px-4 py-3"><span className="font-[family-name:var(--font-inter)] text-sm text-[#e2e3e1]">{inv.email}</span></td>
                      <td className="px-4 py-3"><RoleBadge role={inv.role} /></td>
                      <td className="px-4 py-3"><span className="font-[family-name:var(--font-inter)] text-sm text-[#bab8b7]">{new Date(inv.expires_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}</span></td>
                      <td className="px-4 py-3"><StatusBadge status={inv.status} /></td>
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-3">
                          <button onClick={() => copyLink(inv.id)} className="font-[family-name:var(--font-inter)] text-xs font-bold tracking-[0.08em] uppercase text-[#ff571a] hover:text-white transition-colors">Copy Link</button>
                          <button onClick={() => handleRevoke(inv.id)} disabled={isPending} className="font-[family-name:var(--font-inter)] text-xs font-bold tracking-[0.08em] uppercase text-[#6b7280] hover:text-red-400 transition-colors disabled:opacity-40">Revoke</button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      {canInvite(currentProfile.role) && pastInvites.length > 0 && (
        <section>
          <h2 className="font-[family-name:var(--font-inter)] text-xs font-bold tracking-[0.15em] uppercase text-[#6b7280] mb-4">Past Invitations</h2>
              ...
              <tbody>
                {pastInvites.map((inv) => (
                  <tr key={inv.id} className="border-b border-white/[0.04]">
                    ...
                    <td className="px-4 py-3"><RoleBadge role={inv.role} /></td>
                    <td className="px-4 py-3"><StatusBadge status={inv.status} /></td>
                    <td className="px-4 py-3"><span className="font-[family-name:var(--font-inter)] text-sm text-[#6b7280]">{new Date(inv.accepted_at ?? inv.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 px-4">
          <div className="w-full max-w-md bg-[#111312] border border-red-500/30 p-6">
            <div className="flex items-center gap-4 mb-5">
              ...
              <div>
                <p className="text-[10px] font-bold tracking-[0.15em] uppercase text-red-400 font-[family-name:var(--font-inter)] mb-0.5">Permanent Action</p>
            ...
            <div className="border border-white/[0.06] bg-white/[0.02] p-4 mb-5">
              <p className="font-[family-name:var(--font-inter)] text-sm text-[#bab8b7] mb-1">You are about to permanently delete:</p>
              <p className="font-[family-name:var(--font-inter)] text-sm font-bold text-[#e2e3e1]">{deleteTarget.full_name ?? 'Unknown'} \u2014 {deleteTarget.email}</p>
              <div className="mt-2"><RoleBadge role={deleteTarget.role} /></div>
            </div>
            <p className="font-[family-name:var(--font-inter)] text-xs text-[#6b7280] mb-5 leading-relaxed">
            ...
            <div className="mb-5">
              ...
              <input
                ...
                placeholder="DELETE"
                className="w-full bg-[#1a1c1b] border border-red-500/30 px-4 py-2.5 text-sm text-[#e2e3e1] font-[family-name:var(--font-inter)] focus:outline-none focus:border-red-500/60 placeholder-[#4b5563] tracking-widest"
              />
            </div>
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 bg-red-600 text-white font-[family-name:var(--font-inter)] text-sm font-bold tracking-[0.1em] uppercase py-2.5 hover:bg-red-500 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
                {isPending ? 'Deleting...' : 'Permanently Delete'}
              </button>
              <button
                onClick={() => { setDeleteTarget(null); setDeleteConfirmText('') }}
                className="px-5 border border-white/10 text-[#9ca3af] font-[family-name:var(--font-inter)] text-sm font-bold uppercase hover:text-[#e2e3e1] transition-colors"
              >
                Cancel
              </button>
            </div>
            ...
            {inviteResult ? (
              <div>
                ...
                {inviteResult.alreadyExisted ? (
                  <p className="font-[family-name:var(--font-inter)] text-sm text-[#bab8b7] mb-6">This user already had an account. Their role has been updated immediately \u2014 no link needed.</p>
                ) : (
                  <div>
                    <p className="font-[family-name:var(--font-inter)] text-sm text-[#bab8b7] mb-4">Share this link with the invited person. It expires in 7 days.</p>
                    <div className="flex items-center gap-2 p-3 bg-white/[0.04] border border-white/10 mb-6">
                      ...
                      <button onClick={() => copyLink(inviteResult.token)} className="shrink-0 bg-[#ff571a] text-black font-[family-name:var(--font-inter)] text-xs font-bold uppercase px-3 py-1.5 hover:bg-white transition-colors">{copied ? '\u2713 Copied' : 'Copy'}</button>
                    </div>
                  </div>
                )}
                <button onClick={() => { setShowInviteModal(false); setInviteResult(null) }} className="w-full border border-white/10 text-[#e2e3e1] font-[family-name:var(--font-inter)] text-sm font-bold uppercase px-4 py-2.5 hover:bg-white/[0.04] transition-colors">Close</button>
              </div>
            ) : (
              <div>
                ...
                <div className="flex flex-col gap-4">
                  <div>
                    <label className="block font-[family-name:var(--font-inter)] text-xs font-bold tracking-[0.1em] uppercase text-[#9ca3af] mb-2">Email Address <span className="text-[#ff571a]">*</span></label>
                    <input type="email" value={inviteEmail} onChange={e => setInviteEmail(e.target.value)} placeholder="staff@example.com" className="w-full bg-[#1a1c1b] border border-white/10 px-4 py-2.5 text-sm text-[#e2e3e1] font-[family-name:var(--font-inter)] focus:outline-none focus:border-[#ff571a]/50 placeholder-[#4b5563]" />
                  </div>
                  <div>
                    <label className="block font-[family-name:var(--font-inter)] text-xs font-bold tracking-[0.1em] uppercase text-[#9ca3af] mb-2">Role <span className="text-[#ff571a]">*</span></label>
                    <select value={inviteRole} onChange={e => setInviteRole(e.target.value as AdminRole)} className="w-full bg-[#1a1c1b] border border-white/10 px-4 py-2.5 text-sm text-[#e2e3e1] font-[family-name:var(--font-inter)] focus:outline-none focus:border-[#ff571a]/50 appearance-none">
                      {allowedRoles.map(r => (<option key={r} value={r}>{ROLE_LABELS[r]}</option>))}
                    </select>
                  </div>
                  {inviteError && <p className="text-sm text-red-400 font-[family-name:var(--font-inter)]">{inviteError}</p>}
                  <div className="flex gap-3 pt-2">
                    <button onClick={handleInvite} disabled={isPending || !inviteEmail.trim()} className="flex-1 bg-[#ff571a] text-black font-[family-name:var(--font-inter)] text-sm font-bold tracking-[0.1em] uppercase py-2.5 hover:bg-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed">{isPending ? 'Sending...' : 'Send Invitation'}</button>
                    <button onClick={() => setShowInviteModal(false)} className="px-4 border border-white/10 text-[#9ca3af] font-[family-name:var(--font-inter)] text-sm font-bold uppercase hover:text-[#e2e3e1] transition-colors">Cancel</button>
                  </div>
                </div>
              </div>
            )}
